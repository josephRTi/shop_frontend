...
<!--          <img src="../assets/logo2.svg" alt="Логотип">-->
        ...
        <nav class="not-mobile">
          ...
          <router-link tag="a" to="/shop">ТОВАРЫ</router-link>
          <router-link tag="a" to="/contacts" style="margin-right:40px;">КОНТАКТЫ</router-link>
          <router-link tag="div" class="cart" to="/cart" style="padding-top:30px;">
...
<!--            <span style="font-size: 13px;padding-left:2px;vertical-align:middle">{{$store.getters.getShopItems.length*1 + $store.getters.getShopItems2.length*1}}</span>-->
            <span>{{ this.$store.getters.getCount }}</span>
          </router-link>
          <router-link :to="{name: 'profile'}" tag="div" class="personal_account">
            <img :src="require('@/assets/user.png')" width="20" height="20">
          </router-link>
          <!-- <a id="search">
...
a {
  position: relative;
  margin-top: 5px;
  color: #007CB7; /*задаём цвет ссылки*/
  cursor: pointer;
...
a:after {
  ...
  background-color: #007CB7; /*задаём цвет линии*/
  content: "";
